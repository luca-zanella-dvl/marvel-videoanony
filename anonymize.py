...
from models.common import DetectBackend
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if anonymize_lps:
        # Load lpd model
        lpd_model = DetectBackend(lpd_weights, device=device)
        lpd_stride = lpd_model.stride
        lpd_imgsz = check_img_size(lpd_imgsz, s=lpd_stride)  # check image size

        # Half
        lpd_half = half and device.type != 'cpu'  # half precision only supported by PyTorch on CUDA
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if label in VEHICLES and anonymize_lps:
                        ...
                        # Inference
                        lpd_pred = lpd_model(veh_img, augment=augment)
